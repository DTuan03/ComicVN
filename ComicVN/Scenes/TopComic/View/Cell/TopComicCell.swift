...
protocol TopComicDelegateCell: AnyObject {
    func didTapTopComicCell(indexPath: IndexPath)
}

class TopComicCell: UITableViewCell {
    static let identifier = "TopComicCell"
    weak var delegate: TopComicDelegateCell?
    ...
    @objc func cellTapped() {
        if let indexPath = indexPath {
            delegate?.didTapTopComicCell(indexPath: indexPath)
        }
    }
...
//            make.right.equalToSuperview()
    ...
    func configData(with model: TopComicModel) {

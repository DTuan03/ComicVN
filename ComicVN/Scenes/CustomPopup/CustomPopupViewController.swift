...
class CustomPopupViewController: UIViewController {
    ...
    let titleLabel = LabelFactory.createLabel(text: "Xoá Bookmarks", font: .bold16, textColor: .black, textAlignment: .left)
    let messageLabel = LabelFactory.createLabel(text: "Bạn có chắc chắn xóa toàn bộ Bookmarks chứ, hành động này không thể hoàn tác", font: .regular16, textColor: .black, textAlignment: .left)
    let cancelBtn = ButtonFactory.createButton("Cancel", font: .bold16, textColor: .black, bgColor: .white)
    let okBtn = ButtonFactory.createButton("OK", font: .bold16, textColor: .black, bgColor: .white)
    let lineView: UIView = {
        let view = UIView()
